...
@v1_router.get(
    ...
    description="Returns the latest posture screening analysis.",
)
async def start_screening() -> ScreeningResponse:
    """
    Return the latest posture analysis generated by PoseAnalyzer.

    The camera stream continuously processes frames through MediaPipe.
    This endpoint returns the latest available analysis result.
    """

    data = pose_analyzer_service.get_last_scan()

    formatted = format_screening_response(
        score=data.get("score", 0),
        issue=data.get("issue", INITIAL_ISSUE),
    ...
    try:
        while True:
            ...
            if (
                ...
                logger.info(
                    f"Sending posture update to client: "
                    f"'{issue}' [ws_id:{ws_id}]"
                )
        ...
        logger.info(
            f"WebSocket client disconnected gracefully "
            f"[ws_id:{ws_id}]"
        )

    except Exception as err:
        logger.error(
            f"Error in WebSocket feedback loop "
            f"[ws_id:{ws_id}]: {err}"
        )
